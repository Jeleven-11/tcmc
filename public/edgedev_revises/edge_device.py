                ...
                if self.camera is None:
                    try:
                        ...
                        if not self.cam.isOpened():
                            print("⚠️ Failed to open /dev/video0, trying /dev/video1")
                            self.cam = cv2.VideoCapture(1)
                        if not self.cam.isOpened():
                            print("❌ No usable camera found on /dev/video0 or /dev/video1")
                            self.cam = None  # Important to prevent usage later
                        else:
