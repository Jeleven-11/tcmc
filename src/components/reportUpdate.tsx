        ...
        <div className="space-y-4">
          {currentReports.map((report) => (
            <div key={report.reportID} className="bg-white p-4 rounded shadow-md">
              ...
              <p><strong>Color:</strong> {report.color}</p>
              <p><strong>Reason:</strong> {report.reason}</p>
              {/* <p className="font-bold">Status: <span style={{color: statusColors[report.status]}}>{(report.status.charAt(0).toUpperCase() + report.status.substring(1).replace('_', ' '))}</span></p> */}
